...
- (void)setLongin
{
    ...
    UIImageView *LogImage = [[UIImageView alloc]initWithFrame:CGRectMake(20, 50,150, 40)];
...
//        make.width.mas_equalTo(Meetingbuttonsize.width);
    ...
    _TextField.leftView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"LoginUs"]];
...
//    _PassTextField.leftView.frame = CGRectMake(0, 0, 30, 30);
    _PassTextField.leftView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"LoginPas"]];
...
- (void)changeLanguage:(UIButton *)button
{
    ...
    [chinabutton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    ...
    [Engbutton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
...
- (void)LonginAction:(UIButton *)button
{
    ...
    _mbp.label.text = Localized(@"正在登录，稍后");
    _mbp.backgroundColor = [UIColor colorWithRed:154/255.0 green:116/255.0 blue:79/255.0 alpha:0.2];
    _mbp.mode =  MBProgressHUDModeAnnularDeterminate;
    [_mbp showAnimated:YES whileExecutingBlock:^{
        ...
        {
                _mbp = [MBProgressHUD showHUDAddedTo:[UIApplication sharedApplication].keyWindow animated:YES];
                _mbp.mode = MBProgressHUDModeText;
                _mbp.label.text = Localized(@"网络连接有误，检查N-mix网络配置");
                _mbp.margin = 10.0f; //提示框的大小
                //                  _mbp.backgroundColor = [UIColor whiteColor];
                _mbp.label.textColor = [UIColor blackColor];
                [_mbp customView];
                [_mbp setOffset:CGPointMake(10, 300)];//提示框的位置
                _mbp.removeFromSuperViewOnHide = YES; //隐藏之后删除viewR
                [_mbp hideAnimated:YES afterDelay:1.5]; //多久后隐藏
        }
    }];
